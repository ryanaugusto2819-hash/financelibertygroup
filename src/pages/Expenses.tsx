...
const Expenses = () => {
  const { expenses, selectedDate, dateRange, addAutoExpenses, registerFbAdsPayment, fbAdsAccumulated, fbAdsPaid, updateExpense, deleteExpense } = useFinance();
  ...
  useEffect(() => {
    ...
    if (totalFrete > 0) {
      autoExpenses.push({
        date: today, description: "Frete (automático - LibertyPainel)", category: "Logística",
        amount: totalFrete, type: "variavel", status: "pago", isAutoGenerated: true,
      });
    }
    ...
    if (adsSpend > 0) {
      ...
      if (remaining > 0) {
        autoExpenses.push({
          date: today, description: `FATURA FACEBOOK ADS (${formatCurrency(adsWithTax)} total - ${formatCurrency(fbAdsPaid)} pago)`,
          category: "Facebook ADS", amount: remaining, type: "variavel", status: "agendado", isAutoGenerated: true,
        });
      }
    }
