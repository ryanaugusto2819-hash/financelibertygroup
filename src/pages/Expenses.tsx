...
const Expenses = ({ country }: ExpensesProps = {}) => {
  ...
  useEffect(() => {
    ...
    if (adsBR > 0) {
      const adsWithTax = adsBR * 1.12;
      autoExpenses.push({
        date: today, description: `FATURA FACEBOOK ADS 🇧🇷 (${formatCurrency(adsWithTax)} c/ imposto)`,
        category: "Facebook ADS", amount: adsWithTax, type: "variavel", status: "agendado", isAutoGenerated: true, country: "brasil",
      });
    }

    // Facebook ADS Uruguay
    const adsUY = adsData?.uruguay?.totalSpend ?? 0;
    if (adsUY > 0) {
      const adsWithTax = adsUY * 1.12;
      autoExpenses.push({
        date: today, description: `FATURA FACEBOOK ADS 🇺🇾 (${formatCurrency(adsWithTax)} c/ imposto)`,
        category: "Facebook ADS", amount: adsWithTax, type: "variavel", status: "agendado", isAutoGenerated: true, country: "uruguay",
      });
    }
